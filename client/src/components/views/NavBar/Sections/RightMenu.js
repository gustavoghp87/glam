  ...
  const reCarrito = () => {
    try {
      ...
      user.userData.cart.forEach(element => {
        ...
        //console.log(numCarrito)         // sumar por ítem: user.userData.cart.length
      });
    } catch(e) {};
  };
  ...
  if (user.userData && user.userData.isAuth && user.userData.isAdmin) {
    ...
    return (
      <Menu mode={props.mode}>
        ...
        <Menu.Item key="6" id="cart" style={{padding:'0px 0px 10px 3px'}}>
    ...
    return (
      <Menu mode={props.mode}>
        ...
        <Menu.Item key="10" id="cart" style={{padding:'0px 0px 10px 3px'}}>
          <Badge count={user.userData && numCarrito}>
