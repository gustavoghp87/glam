...
router.get("/auth", auth, async (req, res) => {
    ...
    if (req.cookies.facebook=="true") {
        let user = await User.findOne({fbAccessToken: req.cookies.fbAccessToken});
        //console.log("Encontrado usuario facebook en /auth,", user)
        pack = {
            _id: user._id,
        ...
        let user = await User.findOne({glAccessToken: req.cookies.glAccessToken});
        //console.log("Encontrado usuario facebook en /auth,", user)
        pack = {
            _id: user._id,
            isAdmin: user.role === 0 ? false : true,
...
//    console.log("Para logout:", req.user)
...
//    { dateOfPurchase:1594436510, name:'Faustino Electronic', id: '5f03a4de9309a13c1cbfb200', price:'99.99', quantity: 4, paymentId: 'abcabcabcabc123' }
...
router.post('/procesar-pago', auth, async (req, res) => {
    ...
    var preference = {
        ...
        //     "installments": 12
        // },
        notification_url: "https://glamstudio.com.ar/api/users/notif",
